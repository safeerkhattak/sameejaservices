            ...
            {products.map((product) => (
              <article key={product.id} className={`rounded-2xl border p-5 ${product.isActive ? "border-slate-200 bg-white" : "border-slate-200 bg-slate-50 opacity-75"}`}>
                ...
                <div className="mt-5 grid grid-cols-2 gap-2 border-t border-slate-100 pt-4">
                  <Button variant="outline" className="rounded-xl" onClick={() => setEditing(product)} disabled={busyId === product.id}><Edit3 />Edit</Button>
                  {product.isActive ? (
                    <AlertDialog><AlertDialogTrigger asChild><Button variant="outline" className="rounded-xl border-rose-200 text-rose-700 hover:bg-rose-50" disabled={busyId === product.id} aria-busy={busyId === product.id}>{busyId === product.id ? <Loader2 className="animate-spin" /> : <PackageX />}{busyId === product.id ? "Deactivating…" : "Deactivate"}</Button></AlertDialogTrigger><AlertDialogContent className="rounded-2xl"><AlertDialogHeader><AlertDialogTitle>Deactivate {product.articleName}?</AlertDialogTitle><AlertDialogDescription>Staff will no longer be able to select it on new invoices. Existing invoices will keep their saved article details.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={busyId === product.id}>Keep active</AlertDialogCancel><AlertDialogAction variant="destructive" disabled={busyId === product.id} onClick={() => changeStatus(product, "deactivate")}>{busyId === product.id ? <Loader2 className="animate-spin" /> : <PackageX />}{busyId === product.id ? "Deactivating…" : "Deactivate"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
                  ) : <Button variant="outline" className="rounded-xl border-emerald-200 text-emerald-700 hover:bg-emerald-50" onClick={() => changeStatus(product, "activate")} disabled={busyId === product.id} aria-busy={busyId === product.id}>{busyId === product.id ? <Loader2 className="animate-spin" /> : <PackageCheck />}{busyId === product.id ? "Activating…" : "Activate"}</Button>}
                </div>
              </article>
            ))}
          </div>
      ...
      <Dialog open={editing !== undefined} onOpenChange={(open) => !open && !submitting && setEditing(undefined)}>
        <DialogContent className="rounded-2xl sm:max-w-[600px]">
          ...
          <form key={editing?.id ?? "new"} onSubmit={saveProduct} className="grid gap-4 sm:grid-cols-2">
            ...
            <div><Label htmlFor="unit" className="mb-2 block">Default unit</Label><Input id="unit" name="unit" defaultValue={editing?.unit ?? "Kg"} required maxLength={20} className="h-11 rounded-xl" placeholder="Kg" /><p className="mt-1.5 text-xs leading-5 text-slate-500">Pre-filled on invoices, but it can be changed for each delivery.</p></div>
            ...
            <DialogFooter className="pt-2 sm:col-span-2"><Button type="button" variant="outline" className="rounded-xl" onClick={() => setEditing(undefined)} disabled={submitting}>Cancel</Button><Button type="submit" className="rounded-xl bg-[#2b7a78] hover:bg-[#246b69]" disabled={submitting} aria-busy={submitting}>{submitting ? <Loader2 className="animate-spin" /> : <PackageCheck />}{submitting ? (editing ? "Saving changes…" : "Adding product…") : editing ? "Save changes" : "Add product"}</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
    </>
