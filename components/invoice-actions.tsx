...
export function InvoiceActions({ invoiceId, status, isOwner, demoMode }: { invoiceId: string; status: string; isOwner: boolean; demoMode: boolean }) {
  ...
  const isBusy = busyStatus !== null || isRefreshing;
  ...
  return (
    <div className="flex flex-wrap gap-3 print:hidden">
      ...
      {isOwner && status === "pending_review" && <Button className="h-11 cursor-pointer rounded-xl bg-[#2b7a78] hover:bg-[#246b69]" onClick={() => setStatus("issued")} disabled={isBusy} aria-busy={busyStatus === "issued" || isRefreshing}>{busyStatus === "issued" || isRefreshing ? <Loader2 className="animate-spin" /> : <CheckCircle2 />}{busyStatus === "issued" || isRefreshing ? "Updating…" : "Approve & issue"}</Button>}
      {isOwner && status !== "cancelled" && (
        <AlertDialog>
          <AlertDialogTrigger asChild><Button variant="outline" className="h-11 cursor-pointer rounded-xl border-rose-200 bg-white text-rose-700 hover:bg-rose-50" disabled={isBusy}>{busyStatus === "cancelled" || isRefreshing ? <Loader2 className="animate-spin" /> : <XCircle />}{busyStatus === "cancelled" || isRefreshing ? "Updating…" : "Cancel invoice"}</Button></AlertDialogTrigger>
          <AlertDialogContent className="rounded-2xl">
            ...
            <AlertDialogFooter><AlertDialogCancel disabled={isBusy}>Keep invoice</AlertDialogCancel><AlertDialogAction variant="destructive" disabled={isBusy} onClick={() => setStatus("cancelled")}>{busyStatus === "cancelled" ? <Loader2 className="animate-spin" /> : <XCircle />}{busyStatus === "cancelled" ? "Cancelling…" : "Cancel invoice"}</AlertDialogAction></AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      )}
    </div>
