...
import { getProducts, isDemoMode } from "@/lib/data";

export const dynamic = "force-dynamic";

export default async function NewInvoicePage() {
  const [member, productRows] = await Promise.all([requireMember("/invoices/new"), getProducts({ activeOnly: true })]);
  ...
  return (
    <div className="mx-auto w-full max-w-[1480px] px-5 py-7 sm:px-8 lg:px-10 lg:py-9">
      ...
      <InvoiceForm demoMode={isDemoMode()} products={products} canManageProducts={member.role === "owner"} />
    </div>
  );
}
